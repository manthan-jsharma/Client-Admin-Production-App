...
> = {
  ...
  approved: {
    ...
    icon: CheckCircle2,
    style: {
      background: "rgba(107,207,122,0.12)",
      color: "#16a34a",
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [projRes, delRes, setupRes] = await Promise.all([
        ...
        fetch(`/api/projects/${projectId}/deliveries`, {
          headers: { Authorization: `Bearer ${token}` },
        }).then((r) => r.json()),
        fetch(`/api/projects/${projectId}/setup-items`, {
          headers: { Authorization: `Bearer ${token}` },
        }).then((r) => r.json()),
      ]);
  ...
  const submitDelivery = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        notify("error", result.error || "Failed to submit delivery");
      }
    } catch {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#E9EEF2" }}>
      <PageHeader
        ...
        breadcrumbs={[
          { label: "Dev Portal", href: "/dashboard/dev" },
      ...
      >
          ...
          {TABS.map((tab) => (
            <button
              ...
              style={{
                ...
                boxShadow:
                  activeTab === tab.id
                    ? "0 2px 8px rgba(107,207,122,0.25)"
                    : "none",
              }}
            >
              {tab.label}
          ...
          >
            {[
              ...
              },
              {
                icon: CheckCircle2,
                label: "Status",
                value: project.status.replace(/-/g, " "),
              },
              {
                icon: Calendar,
                label: "Start Date",
              ...
              >
                ...
                >
                  <row.icon
                    style={{ width: 16, height: 16, color: "#6BCF7A" }}
                  />
                </div>
                <div>
                  <p
            ...
            {project.description && (
              <div
                style={{ ...CARD, padding: "16px 20px", gridColumn: "1 / -1" }}
              >
                <p
                  style={{
                    fontSize: 11,
                ...
                <p style={{ fontSize: 13, color: "#5F6B76", lineHeight: 1.6 }}>
                  {project.description}
                </p>
              </div>
            )}
            {project.type === "ai_saas" && (
              <div
                style={{
                  ...
                  padding: "16px 20px",
                  gridColumn: "1 / -1",
                  border: project.uiDesignPreference
                    ? "1px solid #a7f3d0"
                    : "1px solid #DDE5EC",
                }}
              >
                ...
                {project.uiDesignPreference ? (
                  ...
                  >
                    Client hasn't submitted their UI preference yet.
                  </p>
                )}
              </div>
            )}
            ...
            {!project?.roadmap || project.roadmap.length === 0 ? (
              <div style={CARD}>
                <EmptyState
                  ...
                  description="The admin hasn't initialized the roadmap for this project."
                />
              </div>
            ) : (
              <div
              ...
              >
                <div
                  style={{
                    ...
                    padding: "10px 14px",
                    borderRadius: 10,
                    background: "rgba(107,207,122,0.07)",
                    border: "1px solid rgba(107,207,122,0.2)",
                    marginBottom: 4,
                  }}
                >
                  ...
                  />
                  <p style={{ fontSize: 11, color: "#5F6B76" }}>
                    You can add video URLs to each day. All other fields are
                      ...
                      <div
                        style={{
                          ...
                          background: item.completed
                            ? "rgba(107,207,122,0.12)"
                            : "rgba(58,141,222,0.08)",
                          border: `1px solid ${
                            item.completed
                              ? "rgba(107,207,122,0.3)"
                              : "rgba(58,141,222,0.2)"
                          }`,
                        }}
                      >
                        <span
                          style={{
                            fontSize: 11,
                            fontWeight: 800,
                            color: item.completed ? "#16a34a" : "#3A8DDE",
                          }}
                        >
                          D{item.day}
                        </span>
                        ...
                        {editingDay === item.day ? (
                          ...
                          >
                            {item.videoUrl ? (
                              ...
                              >
                                <Film style={{ width: 12, height: 12 }} /> Watch
                                Video
                              </a>
                            ) : (
                              <span
                            ...
                            <button
                              onClick={() => openVideoEdit(item.day)}
                              style={{
                                display: "inline-flex",
                                alignItems: "center",
                                gap: 4,
        ...
        {activeTab === "deliveries" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            >
              <div>
                <h2 style={{ fontSize: 13, fontWeight: 700, color: "#1E2A32" }}>
                  Deliveries
                </h2>
                <p style={{ fontSize: 11, color: "#8A97A3", marginTop: 1 }}>
                  {deliveries.length} deliver
                  {deliveries.length !== 1 ? "ies" : "y"}
                </p>
              </div>
              <button
                onClick={() => setShowDeliveryForm((p) => !p)}
                style={{
                  display: "inline-flex",
                  alignItems: "center",
                  gap: 6,
            ...
            >
              ...
              />
              <p style={{ fontSize: 11, color: "#5F6B76", lineHeight: 1.5 }}>
                Deliveries go to admin for review before the client sees them.
              </p>
            </div>
            ...
            {showDeliveryForm && (
              <div style={{ ...CARD, overflow: "hidden" }}>
                ...
                >
                  <h3
                    style={{ fontSize: 13, fontWeight: 700, color: "#1E2A32" }}
                  >
                    New Delivery #{deliveries.length + 1}
                  </h3>
                </div>
                <form
                ...
                >
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. Initial prototype, Feature v1.2"
                      required
                      style={INPUT}
                    />
                  </div>
                  ...
                  <div style={{ display: "flex", gap: 10 }}>
                    <button
                      type="submit"
                      disabled={
                        submittingDelivery || !deliveryForm.title.trim()
                      }
                      style={{
                        display: "inline-flex",
                        alignItems: "center",
            ...
            {deliveries.length === 0 && !showDeliveryForm ? (
              <div style={CARD}>
                <EmptyState
                  ...
                  description="Click 'Add Delivery' to submit your first delivery."
                />
              </div>
            ) : (
              <div
          ...
          <div>
            {setupItems.length === 0 ? (
              <div style={CARD}>
                <EmptyState
                  variant="projects"
                  title="No setup items"
                  description="The admin hasn't added setup items for this project yet."
                />
              </div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  ...
                  >
                    <div style={{ marginTop: 1, flexShrink: 0 }}>
                      {item.completed ? (
                        ...
                        <Square
                          style={{ width: 16, height: 16, color: "#DDE5EC" }}
                        />
                      )}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p
                        style={{
                          fontSize: 13,
                          fontWeight: 600,
                          color: item.completed ? "#1E2A32" : "#8A97A3",
                        }}
                      >
                        {item.title}
                      </p>
                            ...
                            item.value.includes(".com") ? (
                              <>
                                ...
                                <button
                                  ...
                                  className="p-1 rounded bg-white/50 hover:bg-white text-blue-600 shadow-sm transition-colors flex-shrink-0"
                                  title="Copy link"
                                >
                ...
                <p style={{ fontSize: 11, color: "#8A97A3", marginTop: 4 }}>
                  {setupItems.filter((i) => i.completed).length}/
                  {setupItems.length} items completed
                </p>
              </div>
            )}
          </div>
