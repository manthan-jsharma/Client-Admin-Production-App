...
import {
  ...
  X, FileText, CheckCheck, Check, FolderKanban, ExternalLink,
} from 'lucide-react';
...
function AttachmentBubble({ a, isOwn }: { a: ChatAttachment; isOwn: boolean }) {
  const bubbleStyle: React.CSSProperties = isOwn
    ? { background: 'rgba(107,207,122,0.1)', border: '1px solid rgba(107,207,122,0.25)', color: '#16a34a' }
  ...
  if (a.type === 'voice') return (
    <div className="flex items-center gap-2 px-3 py-2 rounded-xl text-xs" style={bubbleStyle}>
      <Mic className="w-3.5 h-3.5 flex-shrink-0" /><span>{a.filename}</span><span className="opacity-60">{formatSize(a.size)}</span>
      {a.s3Key && !a.s3Key.startsWith('mock://') && <a href={a.s3Key} target="_blank" rel="noopener noreferrer"><ExternalLink className="w-3 h-3 opacity-60" /></a>}
    </div>
  );
  if (a.type === 'video') return (
    <div className="flex items-center gap-2 px-3 py-2 rounded-xl text-xs" style={bubbleStyle}>
      <Video className="w-3.5 h-3.5 flex-shrink-0" /><span>{a.filename}</span><span className="opacity-60">{formatSize(a.size)}</span>
    </div>
  );
  return (
    <div className="flex items-center gap-2 px-3 py-2 rounded-xl text-xs" style={bubbleStyle}>
      <FileText className="w-3.5 h-3.5 flex-shrink-0" /><span className="truncate max-w-[180px]">{a.filename}</span><span className="opacity-60 flex-shrink-0">{formatSize(a.size)}</span>
    </div>
  );
}
...
function renderWithMentions(text: string) {
  return text.split(/(@\w+)/g).map((part, i) => {
    ...
    const color = lower === '@ai' ? '#8b5cf6' : lower === '@admin' ? '#3A8DDE' : lower === '@dev' ? '#16a34a' : '#f59e0b';
    ...
    <div className={`flex items-end gap-2.5 ${isOwn ? 'flex-row-reverse' : 'flex-row'}`}>
      {!isOwn && (
        <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0"
          style={isAI ? { background: 'linear-gradient(135deg,#8b5cf6,#7c3aed)' } : { background: 'linear-gradient(135deg,#3A8DDE,#2F6FB2)' }}>
          {isAI ? <Sparkles className="w-3.5 h-3.5 text-white" /> : <ShieldCheck className="w-3.5 h-3.5 text-white" />}
        </div>
      )}
      <div className={`max-w-[70%] flex flex-col ${isOwn ? 'items-end' : 'items-start'}`}>
        <span className="text-[10px] mb-1 px-1" style={{ color: '#8A97A3' }}>{isAI ? 'AI Assistant' : msg.senderName}</span>
        {msg.message && (
          <div className="px-4 py-2.5 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap"
            style={isOwn
              ? { background: '#6BCF7A', color: 'white', borderBottomRightRadius: '4px', boxShadow: '0 2px 8px rgba(107,207,122,0.2)' }
              : isAI
        ...
        {msg.attachments && msg.attachments.length > 0 && (
          <div className={`mt-1.5 space-y-1 ${isOwn ? 'items-end' : 'items-start'} flex flex-col`}>
            {msg.attachments.map((a, i) => <AttachmentBubble key={i} a={a} isOwn={isOwn} />)}
          </div>
        )}
...
export default function DevChatPage() {
  ...
  useEffect(() => {
    fetch('/api/projects', { headers: { Authorization: `Bearer ${localStorage.getItem('auth_token')}` } })
      .then(r => r.json())
      .then(res => { if (res.success && res.data.length > 0) { setProjects(res.data); setActiveProjectId(res.data[0]._id ?? null); } })
      .catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    setMessages([]);
    fetch(`/api/chats?projectId=${activeProjectId}`, { headers: { Authorization: `Bearer ${localStorage.getItem('auth_token')}` } })
      .then(r => r.json())
      .then(res => { if (res.success) { setMessages(res.data); setLastPollTime(new Date()); markRead(activeProjectId); } })
      .catch(() => {})
  ...
  const poll = useCallback(async () => {
    if (!activeProjectId) return;
    try {
      const res = await fetch(`/api/chats?projectId=${activeProjectId}&since=${lastPollTime.toISOString()}`, {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setMessages(prev => { const ids = new Set(prev.map(m => m._id)); return [...prev, ...newMsgs.filter(m => !ids.has(m._id))]; });
        setNewMessage('');
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await fetch('/api/chat/upload', { method: 'POST', headers: { Authorization: `Bearer ${localStorage.getItem('auth_token')}` }, body: fd });
      const result = await res.json();
  ...
  const completeMention = (handle: string) => {
    ...
    const replaced = newMessage.slice(0, cursor).replace(/@(\w*)$/, handle + ' ');
  ...
  return (
    <div className="h-screen flex flex-col">
      ...
      <div className="px-8 pt-8 pb-5 bg-white flex-shrink-0" style={{ borderBottom: '1px solid #DDE5EC' }}>
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm mt-1" style={{ color: '#5F6B76' }}>
              Communicate with the team · type <span style={{ color: '#8b5cf6', fontWeight: 500 }}>@AI</span> for AI assistance
            </p>
          </div>
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl" style={{ background: 'rgba(107,207,122,0.1)', border: '1px solid rgba(107,207,122,0.3)' }}>
        ...
        {projects.length > 1 && (
          <div className="mt-4 flex items-center gap-1 p-1 rounded-xl w-fit" style={{ background: 'rgba(58,141,222,0.06)', border: '1px solid #DDE5EC' }}>
            {projects.map(p => (
              <button key={p._id} onClick={() => setActiveProjectId(p._id ?? null)}
      ...
      <div className="flex-1 overflow-hidden flex flex-col min-h-0 p-6">
        <div className="flex-1 flex flex-col overflow-hidden"
          style={{ background: '#ffffff', border: '1px solid #DDE5EC', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', borderRadius: '16px' }}>

          {/* Messages */}
          <div className="flex-1 overflow-y-auto p-5 space-y-4" style={{ background: 'rgba(107,207,122,0.04)' }}>
            {isLoading ? (
              <div className="flex items-center justify-center h-full">
                <div className="w-8 h-8 border-2 rounded-full animate-spin" style={{ borderColor: 'rgba(107,207,122,0.2)', borderTopColor: '#6BCF7A' }} />
              </div>
            ) : messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full gap-4 text-center px-8">
                <div className="w-16 h-16 rounded-2xl flex items-center justify-center" style={{ background: 'rgba(107,207,122,0.1)', border: '1px solid rgba(107,207,122,0.2)' }}>
                  <ShieldCheck className="w-8 h-8" style={{ color: '#6BCF7A' }} />
                </div>
                <div>
                  <p className="font-semibold mb-1" style={{ color: '#1E2A32' }}>Start the conversation</p>
                  <p className="text-sm" style={{ color: '#5F6B76' }}>Message the team or type <span style={{ color: '#8b5cf6' }}>@AI</span> for help</p>
                </div>
              </div>
            ) : (
              <>
                {messages.map(msg => <MessageRow key={msg._id} msg={msg} userId={user?._id ?? ''} />)}
                {isAITyping && (
                    ...
                    <div className="flex flex-col items-start">
                      ...
                      <div className="px-4 py-2.5 rounded-2xl rounded-bl-sm flex items-center gap-1" style={{ background: '#f5f3ff', border: '1px solid #ddd6fe' }}>
                        <div className="flex gap-0.5">
                          {[0, 150, 300].map(d => <div key={d} className="w-1.5 h-1.5 rounded-full animate-bounce" style={{ animationDelay: `${d}ms`, background: '#8b5cf6' }} />)}
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <div className="px-4 pt-3 flex gap-2 flex-wrap" style={{ borderTop: '1px solid #DDE5EC' }}>
              {pendingAttachments.map((a, i) => (
                <div key={i} className="flex items-center gap-2 px-2.5 py-1.5 rounded-xl text-xs"
                  style={{ background: 'rgba(107,207,122,0.08)', border: '1px solid rgba(107,207,122,0.2)', color: '#334155' }}>
                  {a.type === 'voice' ? <Mic className="w-3 h-3" /> : a.type === 'video' ? <Video className="w-3 h-3" /> : <FileText className="w-3 h-3" />}
                  <span className="max-w-[120px] truncate">{a.filename}</span>
                  <button onClick={() => setPendingAttachments(prev => prev.filter((_, idx) => idx !== i))} style={{ color: '#8A97A3' }}
          ...
          <div className="p-4" style={{ borderTop: '1px solid #DDE5EC', background: '#ffffff' }}>
            ...
            <div className="flex items-end gap-2">
              <button onClick={() => fileInputRef.current?.click()} disabled={isUploading} title="Attach file"
                className="p-2 rounded-xl transition-colors disabled:opacity-40" style={{ color: '#8A97A3' }}
                ...
                {isUploading
                  ? <div className="w-4 h-4 border-2 rounded-full animate-spin" style={{ borderColor: 'rgba(107,207,122,0.2)', borderTopColor: '#6BCF7A' }} />
                  : <Paperclip className="w-4 h-4" />}
              </button>

              <div className="flex-1 relative">
                {mentionQuery !== null && <MentionPicker query={mentionQuery} onSelect={completeMention} />}
                <textarea ref={textareaRef} value={newMessage} onChange={handleMessageChange} onKeyDown={handleKeyDown}
                  placeholder="Type a message… @admin, @client, @AI" rows={1} disabled={isSending}
                  className="w-full px-4 py-3 rounded-2xl resize-none focus:outline-none text-sm leading-relaxed transition-all"
                  style={{ background: 'rgba(107,207,122,0.06)', border: '1px solid #DDE5EC', color: '#1E2A32', minHeight: '44px' }}
                  onFocus={e => { e.currentTarget.style.borderColor = '#6BCF7A'; e.currentTarget.style.boxShadow = '0 0 0 3px rgba(107,207,122,0.12)'; }}
                  onBlur={e => { e.currentTarget.style.borderColor = '#DDE5EC'; e.currentTarget.style.boxShadow = 'none'; }} />
              </div>

              <button onClick={sendMessage} disabled={isSending || (!newMessage.trim() && pendingAttachments.length === 0)}
                ...
                onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.background = '#5ab868'; }}
                onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = '#6BCF7A'; }}>
                {isSending
            ...
            <p className="text-[10px] mt-2 px-1" style={{ color: '#8A97A3' }}>
              <kbd className="px-1 py-0.5 rounded text-[10px]" style={{ background: '#f1f5f9', color: '#5F6B76' }}>Enter</kbd> to send ·{' '}
              <kbd className="px-1 py-0.5 rounded text-[10px]" style={{ background: '#f1f5f9', color: '#5F6B76' }}>Shift+Enter</kbd> for new line
            </p>
          </div>
        </div>
      </div>

      <input ref={fileInputRef} type="file" accept="audio/*,video/*,image/*,.pdf,.doc,.docx,.txt" onChange={handleFileSelect} className="hidden" />
    </div>
  );
}
