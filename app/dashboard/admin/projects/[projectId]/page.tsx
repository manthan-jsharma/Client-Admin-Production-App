...
export default function AdminProjectDetailPage() {
    ...
    try {
      ...
      if (result.success) {
        ...
        setContentForm({
          ...
          teamSelfieVideoAdminFeedback:
            result.data.teamSelfieVideoAdminFeedback ?? "",
  ...
  const toggleSetupItem = async (itemId: string, current: boolean) => {
    try {
      ...
      if (result.success)
        setSetupItems((prev) =>
          prev.map((i) => (i._id === itemId ? result.data : i))
        );
    } catch {
      /* ignore */
    }
  ...
  const saveSetupEdit = async (itemId: string) => {
    try {
      const res = await fetch(`/api/setup-items/${itemId}`, {
        ...
        body: JSON.stringify({
          title: editSetupForm.title,
          value: editSetupForm.value || null,
        }),
      });
      const result = await res.json();
      if (result.success) {
  ...
  const saveDay = async () => {
    ...
    try {
      const res = await fetch(
        `/api/projects/${projectId}/roadmap/${editingDay}`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
      ...
      if (result.success) {
        setProject((prev) => {
          ...
          return {
            ...prev,
            roadmap: prev.roadmap.map((r) =>
              r.day === editingDay ? { ...r, ...dayForm } : r
            ),
          };
        });
        setEditingDay(null);
  ...
  const saveOverview = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/projects/${projectId}/meta`, {
        ...
        body: JSON.stringify({
          ...overviewForm,
          totalPrice: overviewForm.totalPrice
            ? Number(overviewForm.totalPrice)
            : undefined,
        }),
      });
      const result = await res.json();
  ...
  const createDelivery = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/projects/${projectId}/deliveries`, {
        ...
        body: JSON.stringify({
          ...deliveryForm,
          deliveryNumber: Number(deliveryForm.deliveryNumber),
        }),
      });
      const result = await res.json();
      if (result.success) {
  ...
  const tabs: { id: Tab; label: string }[] = [
      ...
      ? [
          {
            ...
            label:
              totalDays > 0
                ? `Roadmap (${completedDays}/${totalDays})`
                : "Roadmap",
          },
        ]
      : []),
    ...
    ...(project.type === "content_distribution"
      ? [{ id: "content" as Tab, label: "Content Uploads" }]
      : []),
    {
      id: "setup",
      label:
        setupItems.length > 0
          ? `Setup (${setupItems.filter((s) => s.completed).length}/${
              setupItems.length
            })`
          : "Setup",
    },
      ...
      <div className="p-8">
              ...
              {editingOverview ? (
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1.5">
                      <FileUploadField
                        ...
                        onChange={(url) =>
                          setOverviewForm((f) => ({ ...f, contractPDF: url }))
                        }
                        folder="contracts"
                    ...
                    <div className="space-y-1.5">
                      <FileUploadField
                        ...
                        onChange={(url) =>
                          setOverviewForm((f) => ({ ...f, scopePDF: url }))
                        }
                        folder="scopes"
                        accept=".pdf,application/pdf"
                        maxSizeMB={20}
                ...
                <div className="p-6 grid grid-cols-2 md:grid-cols-3 gap-5">
                  ...
                  {project.type === "ai_saas" && (
                    <>
                      <Stat
                        label="UI Design Preference"
                        value={project.uiDesignPreference ?? "Not submitted"}
                      />
                      <FileLink
                        label="Demo Video"
                        url={project.demoVideoS3Key}
                      />
                      <FileLink
                        label="Proof of Code"
                        url={project.proofOfCodeS3Key}
                      />
                    </>
                  )}
                </div>
          ...
          <div className="space-y-3">
            {project.roadmap.length === 0 ? (
              <div style={{ ...CARD, padding: "48px", textAlign: "center" }}>
                ...
                <p className="font-medium mb-1.5" style={{ color: "#5F6B76" }}>
                  Roadmap not initialised
                </p>
                <p className="text-sm mb-5" style={{ color: "#8A97A3" }}>
                  Create the 14-day roadmap so you can add video URLs and
                  progress updates for the client.
                </p>
                <button
              ...
              <>
                <p className="text-xs" style={{ color: "#8A97A3" }}>
                  Click any day to edit its title, description, video URL, and
                  mark it complete. The client can watch the video on their
                  Roadmap page.
                </p>
                      ...
                      <div className="flex items-start justify-between gap-3">
                        ...
                        >
                          {editingDay === item.day ? (
                            ...
                            <>
                              <ChevronDown className="w-4 h-4" /> Edit
                            </>
                          )}
                        </button>
                      </div>
        ...
        {activeTab === "deliveries" && (
          <div className="space-y-5">
            <div className="flex justify-between items-center">
              <p className="text-xs" style={{ color: "#8A97A3" }}>
                Per-delivery approval cards. Client reviews and signs off on
                each delivery.
              </p>
              <button
                  ...
                  <div className="p-5">
                    ...
                    {d.status === "submitted" && (
                      <div className="mb-3 space-y-2">
                        ...
                        >
                          <span style={{ color: "#3A8DDE", fontWeight: 600 }}>
                            ⚡ Dev submission
                          </span>
                        ...
                        {devRevisionId === d._id && (
                          <div className="space-y-2">
                            <textarea
                              ...
                              onChange={(e) =>
                                setDevRevisionNote(e.target.value)
                              }
                            ...
                            <div className="flex gap-2">
                              ...
                              <button
                                onClick={() => setDevRevisionId(null)}
                                className="rounded-xl h-7 px-3 text-xs font-medium"
                                style={{
                                  background: "rgba(58,141,222,0.06)",
                                  color: "#5F6B76",
                                  border: "1px solid #DDE5EC",
                    ...
                    <div className="flex items-center justify-between mb-2">
                      ...
                      {project.hdPhotoS3Key ? (
                        <span
                          style={
                            REVIEW_BADGE[
                              project.hdPhotoStatus ?? "pending_review"
                            ] ?? REVIEW_BADGE.pending_review
                          }
                        >
                          {project.hdPhotoStatus ?? "pending_review"}
                        </span>
                    ...
                    {project.hdPhotoS3Key && (
                      ...
                      >
                        ...
                        <span className="truncate font-mono">
                          {project.hdPhotoS3Key.split("/").pop()}
                        </span>
                      </a>
                    )}
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setContentForm((f) => ({
                              ...f,
                              hdPhotoAdminFeedback: e.target.value,
                            }))
                          }
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      {project.teamSelfieVideoS3Key ? (
                        <span
                          style={
                            REVIEW_BADGE[
                              project.teamSelfieVideoStatus ?? "pending_review"
                            ] ?? REVIEW_BADGE.pending_review
                          }
                        >
                          {project.teamSelfieVideoStatus ?? "pending_review"}
                    ...
                    {project.teamSelfieVideoS3Key && (
                      ...
                      >
                        <ExternalLink className="w-3 h-3 flex-shrink-0" />
                        <span className="truncate font-mono">
                          {project.teamSelfieVideoS3Key.split("/").pop()}
                        </span>
                      </a>
                    )}
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setContentForm((f) => ({
                              ...f,
                              teamSelfieVideoStatus: e.target.value,
                            }))
                          }
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setContentForm((f) => ({
                              ...f,
                              teamSelfieVideoAdminFeedback: e.target.value,
                            }))
                          }
              ...
              <div style={{ ...CARD, overflow: "hidden" }}>
                ...
                <div className="p-6 space-y-3">
                  <FileUploadField
                    ...
                    onChange={(url) =>
                      setContentForm((f) => ({ ...f, aiCloneSampleS3Key: url }))
                    }
              ...
              {(project.domainName ||
                ...
                <div style={{ ...CARD, overflow: "hidden" }}>
                  ...
                  <div className="p-6 grid grid-cols-3 gap-5">
                    <Stat
                      label="Domain Name"
                      value={project.domainName ?? "Not provided"}
                    />
                    ...
                    <div>
                      ...
                      <p className="text-sm" style={{ color: "#334155" }}>
                        {project.designPreferences ?? "Not provided"}
                      </p>
                    </div>
                  </div>
                </div>
        ...
        {activeTab === "setup" && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <p className="text-xs" style={{ color: "#8A97A3" }}>
                {project.type === "ai_saas" ? "14-Day Scope" : "7-Day Scope"} —
                manage the onboarding checklist. Clients see and complete these
                from their Setup tab.
              </p>
              {setupItems.length === 0 && (
                <button
            ...
            {setupItems.length > 0 ? (
                                ...
                                item.value.includes(".com") ? (
                                  <div className="flex items-center gap-2">
                                    <a
                                      ...
                                      target="_blank"
                                      rel="noopener noreferrer"
                                      className="text-xs font-semibold text-blue-500 hover:underline truncate max-w-[250px] flex items-center gap-1"
                                    >
                                      {item.value}
                                    ...
                                    <button
                                      onClick={() => {
                                        ...
                                        // toast.success("Copied to clipboard"); // if you use toast
                                      }}
              ...
              <EmptyState
                icon={
                  <Settings2 className="w-8 h-8" style={{ color: "#8A97A3" }} />
                }
                title="No setup items yet"
                description={`Use "Initialise Defaults" to add the ${
                  project.type === "ai_saas"
                    ? "5 AI SaaS onboarding items (GitHub, hosting, domain, tech stack, API keys)"
                    : "5 content distribution items (brand guidelines, logo, content audit, etc.)"
                }, or add custom ones below.`}
              />
            )}
...
function Stat({
  ...
  return (
    <div>
      <p className="text-xs mb-0.5" style={{ color: "#8A97A3" }}>
        {label}
      </p>
      <p
  ...
  return (
    <div>
      ...
      {url ? (
        ...
        <p className="text-sm" style={{ color: "#8A97A3" }}>
          Not uploaded
        </p>
      )}
    </div>
