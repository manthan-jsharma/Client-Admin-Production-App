        ...
        {msg.type === "ticket" && msg.ticket && (
          ...
          >
            ...
            <div className="flex items-center justify-between">
              <span className="capitalize" style={{ color: "#8A97A3" }}>
                {msg.ticket.type.replace("_", " ")}
              </span>
              <span
                className="font-medium capitalize"
                style={{
                  color:
                    msg.ticket.status === "resolved"
                      ? "#6BCF7A"
                      : msg.ticket.status === "in_progress"
                      ? "#3A8DDE"
                      : "#f59e0b",
                }}
              >
                {msg.ticket.status.replace("_", " ")}
              </span>
            </div>
          </Link>
        )}
            ...
            style={
              isOwn
                ? {
                    ...
                    boxShadow: "0 2px 8px rgba(58,141,222,0.25)",
                  }
                : isAI
                ? {
                    background: "#f5f3ff",
...
export default function AdminChatsPage() {
  ...
  const poll = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/chats?projectId=${selectedThread.projectId}&since=${since}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("auth_token")}`,
          },
            ...
            pt.map((t) =>
              t.projectId === selectedThread.projectId
                ? {
                    ...t,
                    lastMessage: {
                      message: fresh[fresh.length - 1].message,
                      senderName: fresh[fresh.length - 1].senderName,
                      senderRole: fresh[fresh.length - 1].senderRole,
                      createdAt: fresh[fresh.length - 1].createdAt,
                    },
                  }
                : t
            )
  ...
  const markRead = async (projectId: string) => {
    try {
      ...
      setThreads((prev) =>
        prev.map((t) =>
          t.projectId === projectId ? { ...t, unreadCount: 0 } : t
        )
      );
    } catch {
      /* ignore */
  ...
  return (
    <div className="h-screen flex flex-col">
      <div
        className="px-8 pt-8 pb-6 bg-white flex-shrink-0"
      ...
      >
        ...
        <p className="text-sm mt-1" style={{ color: "#5F6B76" }}>
          {threads.length} project conversations · polling every 3s
        </p>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Thread sidebar */}
        <div
          className={`flex-shrink-0 overflow-y-auto w-full lg:w-72 ${
            showThreads ? "flex" : "hidden"
          } lg:flex flex-col`}
          style={{
            borderRight: "1px solid #DDE5EC",
            background: "rgba(58,141,222,0.06)",
          }}
        >
          {/* mobile header */}
          <div
            className="lg:hidden px-4 py-3 flex-shrink-0"
            style={{ borderBottom: "1px solid #DDE5EC" }}
          >
            <p className="text-xs font-semibold" style={{ color: "#8A97A3" }}>
                    ...
                    <div className="flex items-start gap-3">
                      <div className="relative">
                        ...
                        <div
                          ...
                          style={{
                            background: "#6BCF7A",
                            border: "2px solid rgba(58,141,222,0.06)",
                          }}
                        />
                      </div>
                        ...
                        <div className="flex items-center justify-between mb-0.5">
                          ...
                          <div className="flex items-center gap-1.5 flex-shrink-0 ml-1">
                            ...
                            {(thread.unreadCount ?? 0) > 0 && (
                              <span
                                className="flex items-center justify-center min-w-[18px] h-[18px] px-1 text-white text-[10px] font-bold rounded-full"
                                style={{ background: "#f59e0b" }}
                              >
                                {(thread.unreadCount ?? 0) > 9
                                  ? "9+"
                                  : thread.unreadCount}
                              </span>
                            )}
        ...
        {selectedThread ? (
          <div
            className={`flex-1 flex-col overflow-hidden ${
              showThreads ? "hidden lg:flex" : "flex"
            }`}
            style={{ background: "#ffffff" }}
          >
            {/* Chat header */}
            <div
              className="px-4 lg:px-6 py-3.5 flex items-center gap-3"
              style={{
            ...
            >
              ...
              >
                {selectedThread.clientAvatar ? (
                  <img
                    src={selectedThread.clientAvatar}
                    alt={selectedThread.clientName}
                    className="w-9 h-9 rounded-full object-cover"
                  />
                ) : (
                  <span className="text-xs font-bold text-white">
              ...
              <div className="flex-1">
                ...
                >
                  ...
                  <span
                    className="px-1.5 py-0.5 rounded-md text-[10px] capitalize"
          ...
          >
            <div className="text-center">
              <div
                ...
                style={{ background: "#f1f5f9", border: "1px solid #DDE5EC" }}
              >
                <MessageSquare
                  className="w-8 h-8"
                  style={{ color: "#8A97A3" }}
              ...
              <p className="text-sm mt-1" style={{ color: "#8A97A3" }}>
