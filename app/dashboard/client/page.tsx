...
function projectProgress(project: Project): number {
  if (project.roadmap?.length > 0)
    return Math.round(
      (project.roadmap.filter((r) => r.completed).length /
...
const GLASS_CARD: React.CSSProperties = {
  ...
  boxShadow:
    "0 4px 24px rgba(58,141,222,0.08), 0 1px 4px rgba(0,0,0,0.04), inset 0 1px 0 rgba(255,255,255,0.85)",
  borderRadius: "18px",
};
...
export default function ClientDashboard() {
  ...
  // Telegram / AI Notify state
  const [tgConnected, setTgConnected] = useState(false);
  const [tgChatId, setTgChatId] = useState<string | null>(null);
  const [tgDeepLink, setTgDeepLink] = useState<string | null>(null);
  const [tgLoading, setTgLoading] = useState(false);
  const [tgDisconnecting, setTgDisconnecting] = useState(false);
  const [tgTextIdx, setTgTextIdx] = useState(0);
  const [tgTextVisible, setTgTextVisible] = useState(true);
  ...
  const TG_HINTS = [
    ...
    "AI replies on Telegram",
  ];

  useEffect(() => {
    const interval = setInterval(() => {
      setTgTextVisible(false);
      setTimeout(() => {
        setTgTextIdx((i) => (i + 1) % TG_HINTS.length);
        setTgTextVisible(true);
      }, 300);
    }, 2800);
  ...
  const handleTgConnect = async () => {
    // Open window synchronously before async — prevents popup blocker
    ...
    try {
      ...
      if (res.deepLink) {
        ...
        if (win) win.location.href = res.deepLink;
        else window.open(res.deepLink, "_blank");
      } else {
        win?.close();
      }
    } catch {
    ...
    const fetchAll = async () => {
      try {
        ...
        const allDeliveries = await Promise.all(
          loaded.map((p) =>
            fetch(`/api/projects/${p._id}/deliveries`, {
              headers: { Authorization: `Bearer ${token}` },
            })
              .then((r) => r.json())
  ...
  const aiProjects = projects.filter(
    (p) => p.type === "ai_saas" && p.roadmap?.length > 0
  );
  const overallProgress = aiProjects.length
    ? Math.round(
        aiProjects.reduce((s, p) => s + projectProgress(p), 0) /
          aiProjects.length
      )
    : 0;
  const pendingReviews = deliveries.filter(
  ...
  const stats = [
    {
      ...
      display: String(countProjects),
      sub: `${projects.length} assigned`,
      icon: FolderKanban,
      accent: "#3A8DDE",
      accentBg: "rgba(58,141,222,0.1)",
      accentBorder: "rgba(58,141,222,0.2)",
      bar: "linear-gradient(90deg,#3A8DDE,#6FB2F2)",
      spark: seededSpark(projects.length || 3, 0.4),
      sparkColor: "#3A8DDE",
    },
    {
      label: "Overall Progress",
      display: aiProjects.length ? `${countProgress}%` : "—",
      sub: aiProjects.length ? "AI SaaS roadmaps" : "No roadmap yet",
      icon: TrendingUp,
      accent: "#8b5cf6",
      accentBg: "rgba(139,92,246,0.1)",
      accentBorder: "rgba(139,92,246,0.2)",
      bar: "linear-gradient(90deg,#8b5cf6,#a78bfa)",
      spark: seededSpark(overallProgress || 50, 0.3),
      sparkColor: "#8b5cf6",
      donut: true,
      donutColor: "#8b5cf6",
      donutValue: overallProgress,
    },
    {
      label: "Pending Reviews",
      display: String(countReviews),
      sub: pendingReviews > 0 ? "Need your sign-off" : "All caught up ✓",
      icon: CheckSquare,
      accent: "#f59e0b",
      accentBg: "rgba(245,158,11,0.1)",
      accentBorder: "rgba(245,158,11,0.2)",
      bar: "linear-gradient(90deg,#f59e0b,#fbbf24)",
      spark: seededSpark(pendingReviews || 2, 0.5),
      sparkColor: "#f59e0b",
    },
    {
      label: "Unread Messages",
      display: String(countMessages),
      sub: unreadMessages > 0 ? "From your admin" : "No new messages",
      icon: MessageSquare,
      accent: "#6BCF7A",
      accentBg: "rgba(107,207,122,0.1)",
      accentBorder: "rgba(107,207,122,0.2)",
      bar: "linear-gradient(90deg,#6BCF7A,#8FE388)",
      spark: seededSpark(unreadMessages || 1, 0.6),
      sparkColor: "#6BCF7A",
    },
  ];
  ...
  deliveries
    ...
    .forEach((d) => {
      ...
      updates.push({
        ...
        text: `"${d.title}" is ready for your review${
          proj ? ` · ${proj.name}` : ""
        }`,
        time: d.createdAt
  ...
  deliveries
    ...
    .sort(
      (a, b) =>
        new Date(b.signedOffAt ?? 0).getTime() -
        new Date(a.signedOffAt ?? 0).getTime()
    )
    .slice(0, 1)
    .forEach((d) =>
      updates.push({
        icon: CheckCircle2,
        color: "#6BCF7A",
        bg: "rgba(107,207,122,0.1)",
        text: `You signed off on "${d.title}"`,
        time: d.signedOffAt
          ? new Date(d.signedOffAt).toLocaleDateString("en-US", {
              month: "short",
              day: "numeric",
            })
          : "Recently",
    ...
    .forEach((p) => {
      ...
      if (last)
        updates.push({
          ...
          text: `Day ${last.day} of "${p.name}" completed`,
          time: "Recently",
        });
    });
      ...
      (p.roadmap ?? [])
        ...
        .forEach((r) => {
          milestones.push({
            ...
            day: r.day,
            urgent:
              r.day - (p.roadmap ?? []).filter((x) => x.completed).length <= 3,
          });
        });
    });
          ...
          >
            ...
            <span
              style={{
                ...
                fontStyle: "italic",
                whiteSpace: "nowrap",
                opacity: tgTextVisible ? 1 : 0,
                transform: tgTextVisible ? "translateY(0)" : "translateY(-4px)",
            ...
            {tgConnected ? (
              <>
                ...
                >
                  <Link2Off style={{ width: 11, height: 11 }} />
                ...
                return (
                  <div
                    ...
                    onMouseEnter={(e) => {
                      ...
                      ).style.boxShadow = `0 12px 40px rgba(58,141,222,0.13), 0 4px 12px rgba(0,0,0,0.06)`;
                    }}
                    onMouseLeave={(e) => {
                      (e.currentTarget as HTMLElement).style.transform =
                        "translateY(0)";
                  ...
                  >
                    ...
                    <div style={{ padding: "20px 20px 16px" }}>
                      ...
                      >
                        ...
                        >
                          <Icon
                            style={{ width: 16, height: 16, color: s.accent }}
                          />
                        </div>
                        {(s as any).donut ? (
                          <DonutRing
                            progress={(s as any).donutValue}
                            size={44}
                            strokeWidth={4}
                            color={(s as any).donutColor}
                      ...
                      <div style={{ fontSize: 11, color: "#8A97A3" }}>
                        {s.sub}
                      </div>
                    </div>
                  </div>
        ...
        <div>
          ...
          >
            ...
            <Link
              ...
              onMouseEnter={(e) =>
                ((e.currentTarget as HTMLElement).style.color = "#3A8DDE")
              }
              onMouseLeave={(e) =>
                  ...
                  <div
                    ...
                    onMouseEnter={(e) => {
                      (e.currentTarget as HTMLElement).style.transform =
                        "translateY(-2px)";
                      (e.currentTarget as HTMLElement).style.boxShadow =
                        "0 12px 40px rgba(58,141,222,0.12)";
                    }}
                    onMouseLeave={(e) => {
                      (e.currentTarget as HTMLElement).style.transform =
                        "translateY(0)";
                    ...
                    <div style={{ padding: "20px 20px 18px" }}>
                      ...
                      {hasRoadmap ? (
                        <div style={{ marginBottom: 14 }}>
                          ...
                          >
                            <span style={{ fontSize: 11, color: "#8A97A3" }}>
                              Roadmap
                            </span>
                            <div
                              style={{
                          ...
                          >
                            {project.roadmap.filter((r) => r.completed).length}{" "}
                            of {project.roadmap.length} days done
                          </p>
                        </div>
                      ) : (
                        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ ...GLASS_CARD, overflow: "hidden" }}>
            ...
            <div style={{ padding: 20 }}>
              {updates.length === 0 ? (
                ...
                >
                  No updates yet — check back after your admin adds activity.
                </p>
              ) : (
                <div style={{ position: "relative" }}>
                  <div
                    style={{
                      ...
                      bottom: 8,
                      width: 1,
                      background: "rgba(221,229,236,0.8)",
                    }}
                  />
                  <div
                    style={{
                  ...
                  {milestones.slice(0, 4).map((m, i) => (
                    <div
                      ...
                      style={{
                        ...
                        border: "1px solid rgba(221,229,236,0.6)",
                        transition: "background 0.12s",
                        animationDelay: `${i * 70}ms`,
                      }}
                      onMouseEnter={(e) =>
                        ((e.currentTarget as HTMLElement).style.background =
                          "rgba(58,141,222,0.07)")
                      }
                      onMouseLeave={(e) =>
                        ((e.currentTarget as HTMLElement).style.background =
                          "rgba(233,238,242,0.6)")
                      }
                    >
                      <div
                        style={{
                      ...
                      >
                        ...
                        >
                          <Calendar
                            style={{ width: 13, height: 13, color: "#8b5cf6" }}
                          />
                        </div>
                        <div>
                          <p
                          ...
                          <p style={{ fontSize: 11, color: "#8A97A3" }}>
                            {m.projectName} · Day {m.day}
                          </p>
                        </div>
                      </div>
                      <span
