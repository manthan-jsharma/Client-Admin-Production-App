...
const STATUS_CONFIG = {
  pending:  { label: 'Under Review',  badgeStyle: { background: '#fffbeb', color: '#f59e0b', border: '1px solid #fde68a' },    icon: Clock },
  approved: { label: 'Published',     badgeStyle: { background: 'rgba(107,207,122,0.1)', color: '#6BCF7A', border: '1px solid #a7f3d0' },    icon: CheckCircle2 },
  rejected: { label: 'Not Published', badgeStyle: { background: 'rgba(58,141,222,0.06)', color: '#5F6B76', border: '1px solid #DDE5EC' },    icon: ThumbsDown },
};
    ...
    <div className="flex gap-1.5">
      {[1, 2, 3, 4, 5].map(n => (
        <button key={n} type="button" onClick={() => onChange?.(n)}
          ...
          <Star className={`w-6 h-6 transition-colors ${n <= (hover || value) ? 'fill-amber-400 text-amber-400' : ''}`} style={n > (hover || value) ? { color: '#DDE5EC' } : {}} />
        </button>
      ))}
    </div>
  );
...
export default function ClientTestimonialsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/testimonials', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify(form) });
      const result = await res.json();
      if (result.success) {
  ...
  return (
    <div className="min-h-screen" style={{ background: '#E9EEF2' }}>
      <PageHeader
        ...
        breadcrumbs={[{ label: 'Dashboard', href: '/dashboard/client' }, { label: 'Testimonial' }]}
        heroStrip={true}
        actions={
          !hasSubmitted ? (
            <button onClick={() => setShowForm(!showForm)}
              className={showForm ? 'flex items-center gap-2 font-semibold rounded-xl h-10 px-4 text-sm transition-all' : 'btn-primary flex items-center gap-2'}
              style={showForm
      ...
      <div className="p-8 space-y-5">
        ...
        <div style={{ ...CARD, border: '1px solid #c8dff0' }} className="overflow-hidden">
          ...
          <div className="p-5 flex items-start gap-4">
            ...
            <div>
              <p className="text-sm font-bold mb-1" style={{ color: '#1E2A32', fontWeight: 800 }}>Your voice matters</p>
        ...
        {showForm && (
          <div style={CARD} className="overflow-hidden">
            <div className="px-6 py-4" style={{ borderBottom: '1px solid #f1f5f9' }}>
              <h2 className="text-sm font-semibold" style={{ color: '#1E2A32', fontWeight: 800 }}>Write Your Testimonial</h2>
              <p className="text-xs mt-0.5" style={{ color: '#8A97A3' }}>Your review will be visible publicly after approval</p>
            </div>
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              <div className="space-y-2">
                <label className="text-xs font-semibold uppercase tracking-wider" style={{ color: '#5F6B76' }}>Star Rating <span style={{ color: '#ef4444' }}>*</span></label>
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-semibold uppercase tracking-wider" style={{ color: '#5F6B76' }}>Your Testimonial <span style={{ color: '#ef4444' }}>*</span></label>
                  <span className="text-[11px]" style={{ color: '#8A97A3' }}>{form.testimonialText.length}/500</span>
                </div>
                <textarea value={form.testimonialText} onChange={e => { setForm(f => ({ ...f, testimonialText: e.target.value.slice(0, 500) })); setErrors(er => ({ ...er, testimonialText: '' })); }}
                  placeholder="Describe your experience working with us. What made the project successful? What did you appreciate most?" rows={5}
                  ...
                  style={{
                    ...
                    border: errors.testimonialText ? '1px solid #ef4444' : '1px solid #DDE5EC',
                    color: '#1E2A32',
                  }} />
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            {testimonials.map(t => {
              ...
              return (
                <div key={t._id} style={{ ...CARD, border: t.status === 'approved' ? '1px solid #a7f3d0' : '1px solid #DDE5EC' }} className="overflow-hidden transition-all">
                  {t.status === 'approved' && <div className="h-[3px] bg-gradient-to-r from-emerald-400 via-emerald-200 to-transparent" />}
                  <div className="p-6">
                    <div className="flex items-start justify-between gap-4 mb-4">
                      <div className="flex gap-0.5">
                        {[1, 2, 3, 4, 5].map(n => <Star key={n} className={`w-4 h-4 ${n <= t.rating ? 'fill-amber-400 text-amber-400' : ''}`} style={n > t.rating ? { color: '#DDE5EC' } : {}} />)}
                      </div>
                      <span className={`inline-flex items-center gap-1.5 text-[11px] px-2.5 py-1 rounded-full font-semibold flex-shrink-0 ${
                        t.status === 'approved' ? 'pill-info' :
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {t.status === 'approved' && <p className="text-xs flex items-center gap-1" style={{ color: '#6BCF7A' }}><ThumbsUp className="w-3 h-3" /> Published</p>}
                    </div>
                    {t.adminFeedback && (
                      <div className="mt-3 pt-3" style={{ borderTop: '1px solid #f1f5f9' }}>
                        <p className="text-xs" style={{ color: '#5F6B76' }}>Admin note: <span style={{ color: '#334155' }}>{t.adminFeedback}</span></p>
                      </div>
                    )}
                  </div>
                </div>
