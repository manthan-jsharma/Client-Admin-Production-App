        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs" style={{ color: '#8A97A3' }}>{project.status}</span>
            </div>
          </div>
          {project.type === 'ai_saas' && totalDays > 0 && (
            <div className="text-right">
              <p className="text-xs" style={{ color: '#8A97A3' }}>Roadmap progress</p>
              <p className="text-2xl font-bold" style={{ color: '#1E2A32', letterSpacing: '-0.02em' }}>{progress}%</p>
              <p className="text-xs" style={{ color: '#8A97A3' }}>{completedDays}/{totalDays} days</p>
            </div>
          )}
        </div>
          ...
          {tabs.map(t => (
            ...
            >
              ...
              {t.badge != null && t.badge > 0 && (
                <span className="flex items-center justify-center w-4 h-4 text-white text-[10px] font-bold rounded-full" style={{ background: '#f59e0b' }}>
                  {t.badge}
                </span>
              )}
            </button>
      ...
      <div className="p-8">
        ...
        {activeTab === 'overview' && (
          <div className="space-y-5">
            ...
            <div style={{ ...CARD, padding: '20px' }}>
              <p className="text-xs font-semibold uppercase tracking-widest mb-2" style={{ color: '#8A97A3' }}>About This Project</p>
            ...
            {(project.contractPDF || project.scopePDF) && (
              <div style={{ ...CARD, padding: '20px' }}>
                <p className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: '#8A97A3' }}>Documents</p>
        ...
        {activeTab === 'roadmap' && project.type === 'ai_saas' && (
          <div className="space-y-3">
            <p className="text-xs mb-4" style={{ color: '#8A97A3' }}>14-day project roadmap. Each completed day includes a progress video from your admin.</p>
              ...
              >
                <div className="p-5">
                  <div className="flex items-start gap-3">
                    {item.completed
                      ? <CheckCircle2 className="w-5 h-5 flex-shrink-0 mt-0.5" style={{ color: '#6BCF7A' }} />
                      : <Circle className="w-5 h-5 flex-shrink-0 mt-0.5" style={{ color: '#DDE5EC' }} />
                    }
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-xs font-bold" style={{ color: '#8A97A3' }}>DAY {item.day}</span>
                        {item.completed && <span className="text-xs" style={{ color: '#6BCF7A' }}>Complete</span>}
                        {!item.completed && item.videoUrl && <span className="text-xs" style={{ color: '#8A97A3' }}>Video available</span>}
                      </div>
          ...
          <div className="space-y-5">
            {pendingDeliveries > 0 && (
              <div className="flex items-center gap-2 rounded-xl px-4 py-3" style={{ background: '#fffbeb', border: '1px solid #fde68a' }}>
                ...
                <p className="text-sm" style={{ color: '#92400e' }}>You have <strong>{pendingDeliveries}</strong> delivery{pendingDeliveries > 1 ? 'ies' : ''} awaiting your review and sign-off.</p>
              </div>
            )}
            ...
            {deliveries.length === 0 ? (
              ...
              deliveries.map(d => (
                <div
                  key={d._id}
                  style={{
                    background: d.status === 'approved' ? '#f0fdf4' : d.status === 'client_reviewing' ? '#fffbeb' : '#ffffff',
                    border: d.status === 'approved' ? '1px solid #a7f3d0' : d.status === 'client_reviewing' ? '1px solid #fde68a' : '1px solid #DDE5EC',
                ...
                >
                  <div className="p-5">
                    <div className="flex items-start justify-between gap-3 mb-3">
                      ...
                      {d.status === 'approved' && (
                        <div className="flex items-center gap-1.5" style={{ color: '#6BCF7A' }}>
                    ...
                    <p className="text-xs mb-4" style={{ color: '#5F6B76' }}>{d.description}</p>
                    ...
                    {d.proofS3Key && (
                      ...
                      >
                        <FileText className="w-3.5 h-3.5" />
                        <span className="flex-1 truncate">{d.proofS3Key.split('/').pop()}</span>
                        <ExternalLink className="w-3 h-3 flex-shrink-0" />
                      </a>
                    )}

                    {d.clientFeedback && (
                      <p className="text-xs mb-3" style={{ color: '#5F6B76' }}>Your feedback: <span style={{ color: '#334155' }}>"{d.clientFeedback}"</span></p>
                    )}

                    {d.signedOffAt && (
                      <p className="text-xs" style={{ color: '#6BCF7A' }}>Signed off on {new Date(d.signedOffAt).toLocaleDateString()}</p>
                    )}
          ...
          <div className="space-y-6">
            ...
            <div style={{ ...CARD, overflow: 'hidden' }}>
              <div className="px-6 py-4" style={{ borderBottom: '1px solid #f1f5f9', background: 'rgba(58,141,222,0.06)' }}>
                <h2 className="text-sm font-semibold" style={{ color: '#1E2A32', fontWeight: 800 }}>Your Uploads</h2>
                <p className="text-xs mt-0.5" style={{ color: '#8A97A3' }}>Upload your files below — they'll be stored and delivered securely</p>
              </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <label className="text-sm font-medium" style={{ color: '#334155' }}>HD Photo of Yourself</label>
                    </div>
                    {project.hdPhotoStatus && (
                      <span style={UPLOAD_BADGE[project.hdPhotoStatus] ?? UPLOAD_BADGE.pending_review}>
                  ...
                  {project.hdPhotoAdminFeedback && (
                    <p className="text-xs mt-1.5 flex items-start gap-1.5" style={{ color: '#f59e0b' }}>
                ...
                <div className="pt-4" style={{ borderTop: '1px solid #f1f5f9' }}>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    {project.teamSelfieVideoStatus && (
                      <span style={UPLOAD_BADGE[project.teamSelfieVideoStatus] ?? UPLOAD_BADGE.pending_review}>
            ...
            {project.aiCloneSampleS3Key && (
              <div style={{ ...CARD, overflow: 'hidden' }}>
                <div className="px-6 py-4" style={{ borderBottom: '1px solid #f1f5f9', background: 'rgba(58,141,222,0.06)' }}>
                  <h2 className="text-sm font-semibold" style={{ color: '#1E2A32', fontWeight: 800 }}>AI Clone Sample — Your Review</h2>
                </div>
                <div className="p-6 space-y-4">
                  <a
                    ...
                    style={{ background: '#f5f3ff', border: '1px solid #ddd6fe', color: '#8b5cf6' }}
                  >
                    <Video className="w-3.5 h-3.5" />
                    <span className="flex-1 truncate font-mono">{project.aiCloneSampleS3Key.split('/').pop()}</span>
                    <span className="text-[11px]">View sample ↗</span>
                  </a>

                  {project.aiCloneApprovalStatus && project.aiCloneApprovalStatus !== 'pending_review' ? (
                    <div className={`flex items-center gap-2`} style={{ color: project.aiCloneApprovalStatus === 'approved' ? '#6BCF7A' : '#ef4444' }}>
                    ...
                    <form onSubmit={submitAiCloneReview} className="space-y-3">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          style={aiCloneForm.action === 'approved'
                            ? { background: 'rgba(107,207,122,0.1)', color: '#6BCF7A', border: '1px solid #a7f3d0' }
                        ...
                        <button
                          ...
                          className="flex items-center gap-1.5 rounded-xl h-9 px-4 text-sm font-medium transition-all active:scale-95"
                          style={aiCloneForm.action === 'rejected'
            ...
            <div style={{ ...CARD, overflow: 'hidden' }}>
              <div className="px-6 py-4" style={{ borderBottom: '1px solid #f1f5f9', background: 'rgba(58,141,222,0.06)' }}>
                <h2 className="text-sm font-semibold" style={{ color: '#1E2A32', fontWeight: 800 }}>Branding & Delivery Info</h2>
              </div>
              <form onSubmit={submitContentUploads} className="p-6 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <div className="flex items-center gap-1.5 mb-1.5">
                      <Globe className="w-3.5 h-3.5" style={{ color: '#5F6B76' }} />
                      <label className="text-xs font-medium" style={{ color: '#334155' }}>Domain Name</label>
                    </div>
                    <Input
                ...
                <div>
                  <div className="flex items-center gap-1.5 mb-1.5">
                    <Palette className="w-3.5 h-3.5" style={{ color: '#5F6B76' }} />
                    <label className="text-xs font-medium" style={{ color: '#334155' }}>Design Preferences</label>
                  </div>
                  <textarea
        ...
        {activeTab === 'setup' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium" style={{ color: '#1E2A32', fontWeight: 800 }}>
              ...
              {totalSetup > 0 && (
                <div className="flex items-center gap-2">
                  <div className="h-1.5 w-32 rounded-full overflow-hidden" style={{ background: '#DDE5EC' }}>
                    <div
                      className="h-full rounded-full transition-all"
                      style={{ width: `${totalSetup > 0 ? Math.round((completedSetup / totalSetup) * 100) : 0}%`, background: '#6BCF7A' }}
                    />
                  </div>
                  <span className="text-xs font-medium w-8 text-right" style={{ color: '#5F6B76' }}>
            ...
            {setupItems.length === 0 ? (
              <EmptyState
                icon={<ClipboardList className="w-8 h-8" style={{ color: '#8A97A3' }} />}
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <p
                            ...
                            style={{ color: item.completed ? '#8A97A3' : '#1E2A32', textDecoration: item.completed ? 'line-through' : 'none' }}
                          >
                            {item.title}
                          </p>
                        ...
                        {item.completed && item.completedAt && (
                          <span className="text-[10px] flex-shrink-0 mt-1" style={{ color: '#6BCF7A' }}>
...
function StatCard({ label, value }: { label: string; value: string }) {
  return (
    ...
    >
      <p className="text-xs mb-1" style={{ color: '#8A97A3' }}>{label}</p>
      <p className="text-sm font-semibold capitalize" style={{ color: '#1E2A32' }}>{value}</p>
    </div>
  );
}
