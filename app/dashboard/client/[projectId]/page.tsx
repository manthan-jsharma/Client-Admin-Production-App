...
import {
  ...
  FileText, Package, AlertCircle, Globe, Palette, Image,
} from 'lucide-react';
            ...
            {(project.contractPDF || project.scopePDF) && (
              <Card className="bg-slate-800/60 border-slate-700/50 p-5">
                ...
                <div className="flex gap-3">
                  {project.contractPDF && (
                    <div className="flex items-center gap-2 bg-slate-700/50 rounded-lg px-3 py-2 text-xs">
                      <FileText className="w-3.5 h-3.5 text-blue-400" />
                      <span className="text-slate-300">Contract PDF</span>
                      <span className="text-xs text-slate-600">(Available after S3 connected)</span>
                    </div>
                  )}
                  {project.scopePDF && (
                    <div className="flex items-center gap-2 bg-slate-700/50 rounded-lg px-3 py-2 text-xs">
              ...
              deliveries.map(d => (
                <Card key={d._id} className={`border transition-all ${d.status === 'approved' ? 'bg-emerald-500/5 border-emerald-500/20' : d.status === 'client_reviewing' ? 'bg-amber-500/5 border-amber-500/30' : 'bg-slate-800/60 border-slate-700/50'}`}>
                  <div className="p-5">
                    ...
                    {d.proofS3Key && (
                      <div className="flex items-center gap-2 bg-slate-700/40 rounded-lg px-3 py-2 mb-4 text-xs">
                        <FileText className="w-3.5 h-3.5 text-blue-400" />
                        <span className="text-slate-400">Delivery proof attached</span>
                        <span className="text-xs text-slate-600">(Available after S3 connected)</span>
                      </div>
                    )}
          ...
          <div className="space-y-6">
            ...
            <Card className="bg-slate-800/60 border-slate-700/50 overflow-hidden">
              <div className="px-6 py-4 border-b border-slate-700/50 bg-slate-700/20">
                ...
                <p className="text-xs text-slate-500 mt-0.5">Provide your S3 keys once AWS is connected, or paste keys here now</p>
              </div>
